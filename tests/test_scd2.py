...
def _empty_existing() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        columns=[
            "player_id", "current_club_id", "position", "market_value",
            SK_COL, EFFECTIVE_DATE_COLUMN, END_DATE_COLUMN, IS_CURRENT_COLUMN,
        ],
    )
...
class TestNewRows:
    def test_unseen_natural_keys_become_new_versions(self, engine):
        ...
        existing, _ = _run_merge(engine=engine, existing=None, incoming=existing_initial, batch_ts=T1)
        ...
        assert stats == SCD2MergeStats(
            new_versions=1, changed_versions=0,
            unchanged_versions=1, historical_preserved=0,
            total_output_rows=2,
        )
        records = engine.to_records(result)
        by_pid = {r["player_id"]: r for r in records}
...
class TestChangedRows:
    ...
    def test_only_untracked_column_change_is_unchanged(self, engine):
        ...
        existing_initial = pd.DataFrame([
            {"player_id": 100, "current_club_id": 1, "position": "GK",
             "market_value": 10_000, "agent_name": "Alice"},
        ])
        existing, _ = scd2_merge(
            existing_dim=None, incoming=existing_initial,
            natural_key=NATURAL_KEY, tracked_columns=TRACKED,
            surrogate_key_column=SK_COL, batch_timestamp=T1, engine=engine,
        )
        # Same tracked-columns, different agent.
        incoming = pd.DataFrame([
            {"player_id": 100, "current_club_id": 1, "position": "GK",
             "market_value": 10_000, "agent_name": "Bob"},
        ])
        result, stats = scd2_merge(
            existing_dim=existing, incoming=incoming,
            natural_key=NATURAL_KEY, tracked_columns=TRACKED,
            surrogate_key_column=SK_COL, batch_timestamp=T2, engine=engine,
        )
...
class TestUnchangedRows:
    def test_identical_incoming_is_unchanged(self, engine):
        ...
        records = engine.to_records(result)
        assert all(r[IS_CURRENT_COLUMN] is True for r in records)
        assert all(r[EFFECTIVE_DATE_COLUMN] == T1 for r in records)  # original effective dates preserved
...
class TestMixedScenario:
    def test_new_changed_unchanged_and_historical_all_present(self, engine):
        ...
        assert stats == SCD2MergeStats(
            new_versions=1, changed_versions=1,
            unchanged_versions=1, historical_preserved=0,
        ...
        result_t3, stats_t3 = _run_merge(engine=engine, existing=result, incoming=incoming_t3, batch_ts=T3)
...
class TestEdgeCases:
    def test_empty_incoming_leaves_existing_intact(self, engine):
        ...
        result, stats = _run_merge(engine=engine, existing=existing, incoming=empty_incoming, batch_ts=T2)
        # Nothing changes
    ...
    def test_validation_rejects_empty_natural_key(self, engine):
        incoming = _incoming([{"player_id": 1, "current_club_id": 1, "position": "GK", "market_value": 1}])
        with pytest.raises(ValueError, match="natural_key"):
            scd2_merge(
                existing_dim=None, incoming=incoming,
                natural_key=[], tracked_columns=TRACKED,
                surrogate_key_column=SK_COL, batch_timestamp=T1, engine=engine,
            )
    ...
    def test_validation_rejects_empty_tracked_columns(self, engine):
        ...
        with pytest.raises(ValueError, match="tracked_columns"):
            scd2_merge(
                existing_dim=None, incoming=incoming,
                natural_key=NATURAL_KEY, tracked_columns=[],
                surrogate_key_column=SK_COL, batch_timestamp=T1, engine=engine,
            )

    def test_validation_rejects_missing_natural_key_column(self, engine):
        # incoming missing 'player_id'
        incoming = pd.DataFrame([{"some_other_id": 1, "current_club_id": 1, "position": "GK", "market_value": 1}])
...
class TestIdempotencyAndDeterminism:
    ...
    def test_historical_rows_never_mutated(self, engine):
        ...
        result_t2, _ = _run_merge(
            engine=engine, existing=existing,
            incoming=_incoming([
                {"player_id": 100, "current_club_id": 1, "position": "GK", "market_value": 20_000},
            ]),
            batch_ts=T2,
        )
        historical_t2 = [r for r in engine.to_records(result_t2)
                         if r[IS_CURRENT_COLUMN] is False]
        # T3: another change
        result_t3, _ = _run_merge(
            engine=engine, existing=result_t2,
            incoming=_incoming([
                {"player_id": 100, "current_club_id": 1, "position": "GK", "market_value": 30_000},
            ]),
            batch_ts=T3,
        )
        historical_t3 = [r for r in engine.to_records(result_t3)
                         if r[IS_CURRENT_COLUMN] is False]
